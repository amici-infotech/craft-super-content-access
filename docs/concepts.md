...
- **Category group default** — `groupId` set; applies when no element (or structure-parent) policy wins.
- **Product type default** — `productTypeId` set; applies when no element (or structure-parent) policy wins (requires Craft Commerce).
